...
container_runtime: auto                # nerdctl → podman → docker (first found)
...
| Container runtime (one of) | Container isolation | |
| — nerdctl + containerd | Lightest, K8s-native | `brew install nerdctl` / Lima on macOS |
| — podman | Daemonless, rootless | `brew install podman` |
| — docker | Most common | `brew install --cask docker` or OrbStack |
| — OrbStack | Light Docker replacement (macOS) | `brew install --cask orbstack` |
